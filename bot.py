...
def handle_message(user_phone, user_message):
    if user_phone not in user_sessions:
        user_sessions[user_phone] = {
            "history": [],
            "lead": {"phone": None, "name": None, "email": None},
            "collecting": False
        }
    ...
    # Trigger collecting if AI asks for any contact detail
    trigger_words = ["full name", "email address", "phone number", "your name", "your email", "your phone", "name,", "email,", "phone,", "contact details", "details from you", "share your"]
    if any(word in ai_reply.lower() for word in trigger_words):
    ...
    if lead_complete:
        ...
        session["lead"] = {"phone": None, "name": None, "email": None}
...
def extract_lead_info(message, lead):
    ...
    # Extract phone
    phone_match = re.search(r'(\+91|0)?[6-9]\d{9}', message)
    if phone_match and not lead["phone"]:
        lead["phone"] = phone_match.group()

    # Extract name
    if not lead["name"]:
        # Comma format: "Manikandan, email, phone"
        parts = message.split(",")
        if len(parts) >= 2:
